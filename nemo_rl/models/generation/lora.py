...
def patched_load_adapter(self, lora_request: LoRARequestWithCfgAndWeights):
    print("vllm lora is patched successfully")
    try:
        ...
        if isinstance(lora_request, LoRARequestWithCfgAndWeights):
            ...
            # ['check_lora_name', 'clone', 'extra_vocab_size', 'from_local_checkpoint', 'from_lora_tensors', 'get_lora', 'id', 'loras', 'rank']
            # print(f"lora: {lora.get_lora(module_name='model.layers.0.self_attn.o_proj')}")
            print(f"lora in lora.patched_load_adapter: {lora.loras}")
            for name, weight in lora.loras.items():
                print(f"name: {name}, weight lora_a shape: {weight.lora_a.shape}")
